...
import all
from config import DEFAULT_DEBUG_DIR, ensure_dir
from stitch.graphcut import get_stitched_image
from stitch.stitching_function import get_warp
...
def get_true_points(img1, img2, src_pts_RT_8, dst_pts_RT_8, good_num_RT_8, scores_RT8):
    ...
    dst_pts_grid_idx_RT_8 = get_really_index(first_point, X_Axis_vector, Y_Axis_vector, dst_pts_RT_8,
                                             size_RT)
    # todo:##########       RT变换        ##########
    # 返回平均点计算得到的RT矩阵,变换后的左图点src_pts1,右图原点dst_pts,变换前作图中心点list_left,变换后左图中心点list_left_center,变换后中心序号center_grid_idx
    ...
    # 剔除的点
    src1_rejected_points = src_pts_RT_8[false_index]
    dst1_rejected_points = dst_pts_RT_8[false_index]
    ...
    # 剔除的点
    src2_rejected_points = src_real_RT8[false_index]
    dst2_rejected_points = dst_real_RT8[false_index]
    # 变换后剔除后的点
    ...
    # 剔除的点
    src3_rejected_points = src_real_A[false_index]
    dst3_rejected_points = dst_real_A[false_index]
    ...
    # 剔除的点
    src4_rejected_points = src_real_H[false_index]
    dst4_rejected_points = dst_real_H[false_index]
    ...
    # 剔除的点
    src5_rejected_points = src_real_H1[false_index]
    dst5_rejected_points = dst_real_H1[false_index]
    ...
    # 剔除的点
    src6_rejected_points = src_real_H2[false_index]
    dst6_rejected_points = dst_real_H2[false_index]
    ...
    # todo: ##########最后一次剔除(H4)阈值判断可以直接删除了.直接用ransac进行最后一次阈值筛查##########
    real_index = []
    false_index = []
    ...
    # 剔除的点
    src7_rejected_points = src_real_H3[false_index]
    dst7_rejected_points = dst_real_H3[false_index]
    ...
    # 右图原点dst_pts,
    # 变换前作图中心点list_left,
    # 变换后左图中心点list_left_center,
    # 变换后中心序号center_grid_idx
    ...
    # todo:进入到寻找视差的阶段
    # 收集:拼接剔除点矩阵
    rejected_src_points = np.concatenate((src1_rejected_points, src2_rejected_points, src3_rejected_points,
                                          src4_rejected_points, src5_rejected_points, src6_rejected_points,
                                          src7_rejected_points), axis=0)
    rejected_dst_points = np.concatenate((dst1_rejected_points, dst2_rejected_points, dst3_rejected_points,
                                          dst4_rejected_points, dst5_rejected_points, dst6_rejected_points,
                                          dst7_rejected_points), axis=0)
    all.real_index_number = len(src_real_H4)
    src_real_H4, dst_real_H4, src_after_real_H4,Hn,src_pts_left_parallax, dst_pts_right_parallax = find_parallax_points(rejected_src_points, rejected_dst_points, src_real_H4, dst_real_H4, img1, img2, size_H4)
    final_stitch = draw_point(img1, img2, src_real_H4, src_after_real_H4, dst_real_H4, H_left_center,
                           H_right_center,
                           list_left_center, size_H4, xmin, xmax, ymin, ymax)
    return src_real_H4, dst_real_H4, Hn, src_after_real_H4, final_stitch, src_pts_left_parallax, dst_pts_right_parallax
...
def find_parallax_points(rejected_src_points,rejected_dst_points,src_real_H4,dst_real_H4,img1,img2,size_H4):
   # todo:把不接纳的点做第一次RANSAC
    ...
    # one:把RANSAC接受的点加入回原本的内点集,并计算新的H矩阵
    ...
    # two:测试新ransac效果
    """
    H4stitching_oral = get_stitched_image(img2, img1, H4)
    H4stitching_new = get_stitched_image(img2, img1,Hn)
    cv2.imshow("H4stitching_oral",H4stitching_oral)
    cv2.imshow("H4stitching_new",H4stitching_new)
    cv2.waitKey()
    """
    # 全部拒绝点
    # 再拒绝
    ...
    # three:ransac后得到点的效果
    '''
    # H_reject = draw_rejected_point(img1, img2, rejected_src_points, rejected_dst_points, size_H4)
    # H_ransac_reject = draw_rejected_point(img1, img2, src_pts_left_false, dst_pts_right_false, size_H4)
    # H_ransac_accept = draw_rejected_point(img1, img2, src_pts_left_, dst_pts_right_, size_H4)
    # H_ransac_accept_new = draw_rejected_point(img1, img2, accepted_src_points, accepted_dst_points, size_H4)
    '''
...
# todo:第二次RANSAC 变换后的不要的点
    ...
    # cv2.waitKey()
    # for i in range(len(src_pts_left_parallax)):
    #     src_pts_left_parallax[i][0] += t2
    H_ransac_reject_accept_parallax = draw_rejected_point(result_img2, img2, src_pts_left_parallax, dst_pts_right_parallax,
                                                          size_H4)
    H_ransac_reject_parallax = draw_rejected_point(result_img2, img2, src_pts_left_reject_parallax,
                                                   dst_pts_right_reject_parallax, size_H4)
    # todo:想把错误剔除的点算一次H矩阵

    # # # todo:把不接纳的点做第三次RANSAC
    """
    src_pts_left_reject_parallax = np.array(src_pts_left_reject_parallax)
    dst_pts_right_reject_parallax = np.array(dst_pts_right_reject_parallax)
    Hr2, maskr2 = cv2.findHomography(src_pts_left_reject_parallax, dst_pts_right_reject_parallax, cv2.RANSAC,1)
    matchesMask2 = maskr2.ravel().tolist()
    src_pts_left_parallax1 = []
    dst_pts_right_parallax1 = []
    src_pts_left_reject_parallax1 = []
    dst_pts_right_reject_parallax1 = []
    for i in range(src_pts_left_reject_parallax.shape[0]):  # 把经过ransac的点筛出来
        if matchesMask2[i] == 1:
            src_pts_left_parallax1.append(src_pts_left_false[i])
            dst_pts_right_parallax1.append(dst_pts_right_false[i])
        else:
            src_pts_left_reject_parallax1.append(src_pts_left_false[i])
            dst_pts_right_reject_parallax1.append(dst_pts_right_false[i])

    H_ransac_reject_accept_parallax1 = draw_rejected_point(img1, img2, src_pts_left_parallax1, dst_pts_right_parallax1, size_H4, xmin, xmax, ymin, ymax)
    H_ransac_reject_parallax1 = draw_rejected_point(img1, img2, src_pts_left_reject_parallax1, dst_pts_right_reject_parallax1, size_H4, xmin, xmax, ymin, ymax)
    """

    # cv2.imshow("H_reject_all_points", H_reject)
    # cv2.imshow("H_ransac_reject", H_ransac_reject)
    # cv2.imshow("H_ransac_accept", H_ransac_accept)
    # cv2.imshow("H_ransac_accept_new", H_ransac_accept_new)

    # cv2.imshow("H_ransac_reject_accept_parallax", H_ransac_reject_accept_parallax)
    # cv2.imshow("H_ransac_reject_parallax", H_ransac_reject_parallax)

    # cv2.imwrite("/media/miao/喵呜喵呜/小论文实验过程/23.05/8/1-origin_reject_points.png", H_reject)
    # cv2.imwrite("/media/miao/喵呜喵呜/小论文实验过程/23.05/8/2-first_reject_points.png", H_ransac_reject)
    # cv2.imwrite("/media/miao/喵呜喵呜/小论文实验过程/23.05/8/3-first_accept_points.png", H_ransac_accept)
    # cv2.imwrite("/media/miao/喵呜喵呜/小论文实验过程/23.05/8/4-second_reject_points.png", H_ransac_reject_parallax)
    # cv2.imwrite("/media/miao/喵呜喵呜/小论文实验过程/23.05/8/5-second_accept_points.png", H_ransac_reject_accept_parallax)

    # cv2.imshow("H_ransac_reject_accept_parallax1", H_ransac_reject_accept_parallax1)
    # cv2.imshow("H_ransac_reject_parallax1", H_ransac_reject_parallax1)
   ...
   # todo:进行正确点特征点变换
    ...
    for i in range(len(accepted_src_points)):
        src_after_real_H4[i][0] = Hn[0][0] * accepted_src_points[i][0] + Hn[0][1] * accepted_src_points[i][1] + Hn[0][2]
        src_after_real_H4[i][1] = Hn[1][0] * accepted_src_points[i][0] + Hn[1][1] * accepted_src_points[i][1] + Hn[1][2]
        den = Hn[2][0] * accepted_src_points[i][0] + Hn[2][1] * accepted_src_points[i][1] + Hn[2][2]
        src_after_real_H4[i][0] = src_after_real_H4[i][0] / den
        src_after_real_H4[i][1] = src_after_real_H4[i][1] / den
    ...
    # for i in range(len(src_pts_left_parallax)):
    #     src_pts_left_parallax[i][0] += t2
    H_ransac_reject_accept_parallax = draw_rejected_point(img1, img2, accepted_src_points, accepted_dst_points,
                                                          size_H4)

    debug_dir = ensure_dir(DEFAULT_DEBUG_DIR)
    cv2.imwrite(str(Path(debug_dir) / "ransac_reject_accept_parallax.png"), H_ransac_reject_accept_parallax)
    H_ransac_reject_parallax = draw_rejected_point(warp_all, img2, src_pts_left_reject_parallax,
                                                   dst_pts_right_reject_parallax, size_H4)
    # todo:想把错误剔除的点算一次H矩阵
    ...
    # cv2.imshow("H_ransac_reject_parallax1", H_ransac_reject_parallax1)
    # cv2.waitKey()
    all.final_residuals = get_adv_residual(src_after_real_H4, accepted_dst_points)
    return accepted_src_points, accepted_dst_points, src_after_real_H4,Hn,src_pts_left_parallax, dst_pts_right_parallax
...
def get_A_transform(src_pts, dst_pts, dst_pts_grid_idx_A):
    ...
    for i in range(idx2):
        ...
        if cnt != 0:
            ...
            list_left.append(seed_left[i])
            list_right.append(seed_right[i])
    list_left = np.array(list_left).reshape(-1, 1, 2)

    list_right = np.array(list_right).reshape(-1, 1, 2)
    # 得到<=9对的种子点list_left list_right 并以此计算A
    Affine_matrix, _ = cv2.estimateAffine2D(list_left, list_right, method=cv2.LMEDS)
    # Affine_matrix, _ = np.array(cv2.estimateAffine2D(list_left, list_right, method=cv2.LMEDS))
    # print(Affine_matrix)
    # 将左图的所有特征点做A变换
    list_left = np.array(list_left.reshape(list_left.shape[0], 2))
    list_right = np.array(list_right.reshape(list_right.shape[0], 2))
    src_pts1 = np.zeros(src_pts.shape)
    ...
    for i in range(list_left.shape[0]):
...
def get_H_transform(src_pts, dst_pts, src_pts_grid_idx, size):
    ...
    # 采用svd分解进行加速计算H
    Homegraphy1, _ = cv2.findHomography(list_left, list_right)
...
def draw_point(src1, src2, src_pts, src_pts2, dst_pts2, RT_left_center, RT_right_center, RT_after_left_center, size,
    ...
    # cv2.line(output, (0, 0), (511, 511), (0,0,255), 5)
    # 画分割线
    ...
    #     cv2.line(output, (x7 + src1.shape[1], ymin), (x7 + src1.shape[1], ymax), (0, 0, 0), 2)
    #     y7 = int(height / math.sqrt(size) + y7)
    #     cv2.line(output, (int(src1.shape[1] + xmin), y7), (int(src1.shape[1] + xmax), y7), (0, 0, 0), 2)

    # cv2.line(output, (int(xmin + src1.shape[1]), int(ymin)), (int(xmax + src1.shape[1]), int(ymin)), (0, 0, 0), 2)
    # cv2.line(output, (int(xmin + src1.shape[1]), int(ymin)), (int(xmin + src1.shape[1]), int(ymax)), (0, 0, 0), 2)
    # cv2.line(output, (int(xmin + src1.shape[1]), int(ymax)), (int(xmax + src1.shape[1]), int(ymax)), (0, 0, 0), 2)
    # cv2.line(output, (int(xmax + src1.shape[1]), int(ymin)), (int(xmax + src1.shape[1]), int(ymax)), (0, 0, 0), 2)
    return output


def draw_rejected_point(src1, src2, rejected_src_points, rejected_dst_points, size):
    size = size.height * size.width
    x7 = 0
    y7 = 0
    line = int(math.sqrt(size) - 1)
    ...
    residual = 0
    # 描点
    for i in range(len(rejected_src_points)):
        # (x1,y1)左图被剔除的点 (x3,y3)右图被剔除的点
        x1 = int(rejected_src_points[i][0])
        y1 = int(rejected_src_points[i][1])
        x2 = int(rejected_dst_points[i][0] + src1.shape[1])
        y2 = int(rejected_dst_points[i][1])
        cv2.circle(output, (x1, y1), 10, (0, 0, 255), -1)
        cv2.circle(output, (x2, y2), 10, (0, 0, 255), -1)

        cv2.line(output, (x2, y2), (x1, y1), (255, 255, 255), 1)
