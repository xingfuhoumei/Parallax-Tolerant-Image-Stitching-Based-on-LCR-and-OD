...
from config import DEFAULT_COMPARE_IMG1, DEFAULT_COMPARE_IMG2, DEFAULT_COMPARE_OUTPUT, ensure_dir

def parse_args():
    parser = argparse.ArgumentParser(description="Run compare pipeline.")
    parser.add_argument("--img1", default=DEFAULT_COMPARE_IMG1, help="Path to reference image.")
    parser.add_argument("--img2", default=DEFAULT_COMPARE_IMG2, help="Path to candidate image.")
    parser.add_argument("--output", default=DEFAULT_COMPARE_OUTPUT, help="Output directory.")
    parser.add_argument("--scale", type=float, default=0.25, help="Resize scale for input images.")
    parser.add_argument("--ratio-test", type=float, default=0.7, help="Ratio test threshold.")
...
if __name__ == '__main__':
    ...
    cv2.imwrite(str(output_dir / "0-reference.png"), img1)
    img2 = cv2.imread(args.img2)
    cv2.imwrite(str(output_dir / "0-candidate2.png"), img2)
    # 图片尺寸比例
